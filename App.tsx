...
const Header: React.FC<{ versionInfo: VersionInfo | null }> = ({ versionInfo }) => {
  ...
  useEffect(() => {
    ...
    // Bolt Optimization: Use passive: true to improve scrolling responsiveness
  ...
  return (
    <div className="absolute inset-0 overflow-hidden z-0 select-none pointer-events-none">
      ...
      <div className="absolute -inset-40 flex justify-center gap-6 -rotate-12 scale-125 opacity-60">
        ...
        {/* Column 1 (Up) */}
        {/* Bolt Optimization: Promote marquee columns to GPU compositor layers with will-change-transform */}
        <div className="flex flex-col gap-6 animate-marquee-up min-h-full shrink-0 will-change-transform">
          {column1.map((src, i) => (
            <div key={`c1-${i}`} className="w-48 h-[300px] shrink-0 rounded-xl overflow-hidden border border-white/10 shadow-2xl bg-slate-900">
              {/* Bolt Optimization: decoding="async" prevents decoding from blocking the main thread */}
