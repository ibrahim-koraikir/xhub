...
const InstallModal: React.FC<{ isOpen: boolean; onClose: () => void }> = ({ isOpen, onClose }) => {
  ...
  React.useEffect(() => {
    if (isOpen) {
      // Small delay to ensure the modal is painted and avoid focus yanking
      ...
      >
        <div className="p-8 md:p-12">
          <button
            ...
            className="absolute top-6 right-6 w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-white hover:bg-white/10 focus-visible:ring-2 focus-visible:ring-brand-500 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-900 outline-none transition-all"
          >
...
function App() {
  ...
  return (
    <div className="min-h-screen bg-black text-white font-sans selection:bg-brand-500/30 selection:text-white">
      <a
        href="#main-content"
        className="sr-only focus:not-sr-only fixed top-4 left-4 z-[60] bg-white text-black px-6 py-3 rounded-full font-bold shadow-2xl transition-all"
      >
        Skip to content
      </a>
